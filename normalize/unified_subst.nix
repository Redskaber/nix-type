# normalize/unified_subst.nix — Phase 4.5.4
  ...
  applySubstToConstraint = usubst: c:
    if !builtins.isAttrs c then c
      ...
      if tag == "Eq" || tag == "Equality" then
        ...
        c // { sub = applySubst usubst c.sub; sup = applySubst usubst c.sup; }
      else if tag == "HasField" then
        c // {
          fieldType = applySubst usubst c.fieldType;
          recType   = applySubst usubst c.recType;
        }
      else c;
